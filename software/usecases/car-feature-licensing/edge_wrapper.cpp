...
int edge_init(Keystone::Enclave* enclave) {
  ...
  register_call(OCALL_WAIT_FOR_SERVER_PUBKEY, wait_for_server_pubkey_wrapper);
  register_call(OCALL_SEND_REPLY, send_reply_wrapper);
  register_call(OCALL_GET_CURRENT_FEATURE_UID, get_current_feature_uid_wrapper);
  register_call(OCALL_PASS_LICENSE_DATA, pass_license_data_wrapper);
...
void wait_for_server_pubkey_wrapper(void* buffer) {
  ...
  void* pubkey = wait_for_server_pubkey();
...
void get_current_feature_uid_wrapper(void *buffer) {
  ...
  int32_t *ptr_ftr_uid = get_current_feature_uid();

  // This handles wrapping the data into an edge_data_t and storing it
  // in the shared region.
  if( edge_call_setup_wrapped_ret(edge_call, ptr_ftr_uid, sizeof(*ptr_ftr_uid))){
    edge_call->return_data.call_status = CALL_STATUS_BAD_PTR;
  }
  else{
    edge_call->return_data.call_status = CALL_STATUS_OK;
...
void pass_license_data_wrapper(void* buffer) {
  ...
  pass_license_data((ftr_license_t*)call_args);
  edge_call->return_data.call_status = CALL_STATUS_OK;
