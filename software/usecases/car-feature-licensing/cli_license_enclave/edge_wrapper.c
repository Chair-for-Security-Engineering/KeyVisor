...
void ocall_wait_for_server_pubkey(unsigned char* pk, size_t len) {
  ocall(OCALL_WAIT_FOR_SERVER_PUBKEY, NULL, 0, pk, len);
  return;
}
...
void ocall_get_current_feature_uid(struct edge_data *msg) {
  ocall(OCALL_GET_CURRENT_FEATURE_UID, NULL, 0, msg, sizeof(struct edge_data));
}

void ocall_pass_license_data(ftr_license_t *feature_license_data) {
  if (!feature_license_data) return;
  ocall(OCALL_PASS_LICENSE_DATA, feature_license_data, sizeof(ftr_license_t), 0, 0);
  return;
}