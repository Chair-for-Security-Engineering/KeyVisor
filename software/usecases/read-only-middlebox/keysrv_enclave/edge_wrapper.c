...
#ifndef KEYSRV_ENC_DEBUG_PRINTS
  return;
#endif
...
void ocall_pass_key_handle(sess_kbundle_t *sess_key_bundle) {
  if (!sess_key_bundle) return;
  ocall(OCALL_PASS_SESSION_KEY_BUNDLE, sess_key_bundle, get_sess_bundle_size(sess_key_bundle->num_handles), 0, 0);
  return;
}

void ocall_wait_for_tcp_client() {
  ocall(OCALL_WAIT_FOR_TCP_CLIENT, NULL, 0, 0, 0);
  return;
}