...
/* boxed messages will include cipher||tag||nonce, where cipher is based on
 * padded plaintext (afaik); */
size_t channel_get_send_size(size_t len) {
  return crypto_secretbox_MACBYTES + BLOCK_UP(len) + crypto_secretbox_NONCEBYTES;
}

/* auth-encrypt msg; buffer will include cipher||tag||nonce;
 * plaintext might be padded before crypting; resulting length of buffer is
 * given by channel_get_send_size(len) which will calculate that based on the
 * plaintext len */
