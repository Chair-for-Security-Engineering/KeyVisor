...
void edge_init(Keystone::Enclave* enclave) {
  ...
  register_call(OCALL_PASS_SESSION_KEY_BUNDLE, pass_key_handle_wrapper);
  register_call(OCALL_WAIT_FOR_TCP_CLIENT, wait_for_tcp_client_wrapper);
...
void pass_key_handle_wrapper(void* buffer) {
  ...
  pass_key_handle((sess_kbundle_t*)call_args);
  edge_call->return_data.call_status = CALL_STATUS_OK;
