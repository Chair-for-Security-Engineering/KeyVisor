...
class Lfsr(n: Int, seed: UInt = 1.U) extends Module {
  assert(seed =/= 0.U, "Seed cannot be zero")

  val io = IO(new PrngIo(n))

  val shiftReg = RegInit(VecInit((seed(n, 0)).asBools))
  when(io.enable){
...
class Lfsr128 extends Lfsr(128)
class Lfsr96 extends Lfsr(96, "h7b0e8ed4a01fabefa8f85421".U)